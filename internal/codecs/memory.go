...
func (c *MemoryCodec) Encode(
	...
	case types.OptionalMemory:
		...
		return encodeOptional(w, !ok, required,
			...
			func() error {
				return missingValueError("geltypes.OptionalMemory", path)
			})
	...
	default:
		return fmt.Errorf("expected %v to be geltypes.Memory, "+
			"geltypes.OptionalMemory or MemoryMarshaler got %T", path, val)
	}
}
