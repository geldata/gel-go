...
func parseDurationISO(str string) (Duration, error) {
	if !strings.HasPrefix(str, "PT") {
		return 0, fmt.Errorf("could not parse geltypes.Duration from %q", str)
	}
	...
	if match != "" {
		...
		if err != nil {
			return 0, fmt.Errorf(
				"could not parse geltypes.Duration from %q: %w",
				str, err)
		}
...
func parseDurationHuman(str string) (Duration, error) {
	...
	if !found {
		return 0, fmt.Errorf(
			"could not parse geltypes.Duration from %q: no duration found",
			str)
	}

	if strings.TrimSpace(s) != "" {
		return 0, fmt.Errorf(
			"could not parse geltypes.Duration from %q: extra characters %q",
			str,
			strings.TrimSpace(s),
		)
	}
...
func (rd *RelativeDuration) UnmarshalText(b []byte) error {
	...
	if !strings.HasPrefix(str, "P") {
		...
		if err != nil {
			return fmt.Errorf(
				"could not parse geltypes.RelativeDuration from %q: %w",
				str, err)
		}
		found = found || f
		...
		if !found {
			return fmt.Errorf(
				"could not parse geltypes.RelativeDuration from %q: "+
					"no duration found",
				str)
		}
...
func (dd *DateDuration) UnmarshalText(b []byte) error {
	...
	if !strings.HasPrefix(str, "P") {
		...
		if err != nil {
			return fmt.Errorf(
				"could not parse geltypes.DateDuration from %q: %w",
				str, err)
		}
		found = found || f
		...
		if !found {
			return fmt.Errorf(
				"could not parse geltypes.DateDuration from %q: "+
					"no duration found",
				str)
		}
	...
	if len(strs) == 2 {
		...
		if s != "" {
			return fmt.Errorf(
				"could not parse geltypes.DateDuration from %q", str)
		}
	}
	...
	if err != nil {
		return fmt.Errorf(
			"could not parse geltypes.DateDuration from %q: %w", str, err)
	}
	...
	if date != "" {
		return fmt.Errorf("could not parse geltypes.DateDuration from %q", str)
	}
